...
public class CommandSender {
    ...
    public Response send(Request request) throws IOException, ClassNotFoundException {
        ...
        try (DatagramChannel channel = DatagramChannel.open()) {
            ...
            channel.register(selector, SelectionKey.OP_WRITE); // сначала отправка, запись в канал данных из буфера
            ...
            for (int attempt = 0; attempt < maxRetries; attempt++) {
                ...
                while (keys.hasNext()) {
                    ...
                    if (key.isWritable()) {
                        ...
                        channel.register(selector, SelectionKey.OP_READ);// теперь чтение из канала в буфер
                    }
                }
                ...
                while (System.currentTimeMillis() - startTime < timeoutMs) {
                    if (selector.select(timeoutMs) > 0) {
                        ...
                        while (it.hasNext()) {
                            ...
                            it.remove();
                            if (key.isReadable()) {
                                from = ((DatagramChannel) key.channel()).receive(recvBuffer);//данные записываются в буфф
